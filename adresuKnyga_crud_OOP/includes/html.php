...
function frontPanel ($limit, $conn) {
    ?>
<div class="wrapper">
    ...
    <?php
    $query="select * from kontaktai";
    $record_per_page=3;
    $query2=$conn ->paging($query,$record_per_page);

    $conn->dataview($query2);
    ?>
    <tr>
        <td colspan="7" align="center">
            <div class="pagination-wrap">
                <?php   $conn->pagelink($query, $record_per_page);    ?>
...
function table ($result, $offset, $limit)
{
    ...
    <div class="w-75 p-3 mx-auto">
            ...
            if (!empty($result)) {
                ...
                for ($i = 0; $i<$totalRows; $i++) {
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post">
                                <button name="edit" value="<?php echo $id; ?>" id="edit" type='submit'
                                        class='btn btn-warning btn-lg tableButton'><span><i
                                                class='fas fa-edit'></i></span></button>
                                <button name="delete" value="<?php echo $id; ?>" id="delete" type='submit'
                                        class='btn btn-danger btn-lg tableButton'><span><i class="fas fa-trash-alt"></i></span>
                                </button>
                            </form>
                        </td>
                    </tr>
        ...
        <?php
        if (empty($result)) {
            echo "<h3 class='text-center'>Nėra duomenų</h3>";
        }
        ?>
    </div>
    <?php
...
function pagesList($result, $limit, $offset, $offsetLink, $conn) {




    $contacts = new ContactModel($conn);
    ...
    <nav>
        <ul id="pagination" class="pagination float-right">
            ...
            $number = 0;
            while ($contacts->totalRows()-($limit*$number) > 0) {
                $sk = "";
